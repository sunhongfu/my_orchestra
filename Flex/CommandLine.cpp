...
bool CommandLine::SeriesNumber(int& specifiedSeriesNumber)
{
    bool wasValueSpecifiedOnCommandLine = false;
    ...
    int localValueToFill = 0;
    if( programOptions.Get<int>("series", localValueToFill) )
    {
        specifiedSeriesNumber = localValueToFill;
        wasValueSpecifiedOnCommandLine = true;
    }

    return wasValueSpecifiedOnCommandLine;
}
...
boost::optional<std::string> CommandLine::KacqFile()
{
    // Check if option has already been registered as
    // the same option cannot be added twice.
    const GESystem::ProgramOptions programOptions;

    if(!programOptions.Has("kacq"))
    {
        boost::program_options::options_description options;

        options.add_options()
            ("kacq", boost::program_options::value<std::string>(), "Kacq file");
    
        programOptions.AddOptions(options);
    }

    return programOptions.Get<std::string>("kacq");
}

bool CommandLine::RunSimple()
{
    boost::program_options::options_description options;

    options.add_options()
        ("simple", boost::program_options::bool_switch(), "Call RunFlexSimple() instead of RunFlex()");

    const GESystem::ProgramOptions programOptions;
    programOptions.AddOptions(options);

    return programOptions.SwitchEnabled("simple");
}
